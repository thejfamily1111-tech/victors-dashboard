...
# -------------------------------------------------------------
# Structure: {ticker: {"last_1h_bar": ts, "last_4h_bar": ts, "data_1h": df, "data_4h": df, "fetched_at": time}}
HTF_BAR_CACHE = {}
...
def fetch_completed_htf_bars(
  ...
  # Check in-memory cache TTL (minimum 3 minutes between network polls)
  ...
  try:
    ...
    if isinstance(raw_1h.columns, pd.MultiIndex):
      raw_1h.columns = [c[0] for c in raw_1h.columns]
    ...
    df_1h["ATR14"] = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1).rolling(
        14
    ).mean()
...
def find_swing_pivot_zones(df_4h: pd.DataFrame) -> tuple[list, list]:
  ...
  # Confirmed 5-bar pivot: H[i] > H[i-1], H[i-2], H[i+1], H[i+2]
...
def evaluate_macro_clearance(
  ...
  # FAIL CLOSED: Missing or incomplete risk data blocks entry
  ...
  r_unit = max(abs(spot_price - stop_price), 0.25)  # 1R distance in dollars

  # --- 1H METRICS (Smoothed & ATR-Buffered) ---
  ...
  if is_call:
    if c_1h < (ema_1h - ema_buffer) and ema_slope_3bar < 0:
      ...
      reason = (
          f"Price (${c_1h:.2f}) < 1H EMA20 - 0.2ATR with negative slope"
          f" ({ema_slope_3bar:+.2f})"
      )
    ...
    if c_1h > (ema_1h + ema_buffer) and ema_slope_3bar > 0:
      ...
      reason = (
          f"Price (${c_1h:.2f}) > 1H EMA20 + 0.2ATR with positive slope"
          f" ({ema_slope_3bar:+.2f})"
      )
    elif c_1h > ema_1h or ema_slope_3bar > 0:
      verdict = "DOWNGRADE"
  ...
  # 2. Check 4H Obstacle Runway (Overrides / Clamps 1H Verdict)
  if obstacle_dist_r < 1.0:
    verdict = "VETO"
    scalar = 0.0
    code = "4H_OBSTACLE_WITHIN_1R"
    reason = (
        f"4H pivot wall at ${nearest_obstacle:.2f} is only {obstacle_dist_r:.2f}R"
        " away (< 1.0R)"
    )
    ...
    reason = (
        f"4H obstacle at ${nearest_obstacle:.2f} sits at {obstacle_dist_r:.2f}R"
        " (blocks clear +2R target)"
    )
...
def get_market_regime():
  ...
  try:
    ...
    if current_vix < 14.0:
      ...
      regime = (
          "HIGH VOL TRENDING" if abs(vix_pct_change) < 5.0 else "HIGH VOL CHOP"
      )
...
def calculate_relative_strength(
  ...
  try:
    ...
    r_sec = (
        float(data[sector_etf].iloc[-1] - data[sector_etf].iloc[-1 - periods])
