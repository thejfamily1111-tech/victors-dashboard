...
Enhanced with Zarattini et al. (2024) Multi-ORB Telemetry & Brooks Bar Anatomy.
"""
...
def create_candidate_record(
    ...
    # Dossier multi-timeframe ORB & Wyckoff extensions
    orb5_high: float = 0.0,
    orb5_low: float = 0.0,
    orb15_high: float = 0.0,
    orb15_low: float = 0.0,
    accepted_or_high: float = 0.0,
    accepted_or_low: float = 0.0,
    extension_r: float = 0.0,
    pattern_setup: str = "IN_RANGE",
    is_trap: bool = False,
) -> dict:
    ...
    if direction.upper() in {"CALL", "LONG"}:
        stop_ref = round(spot - risk_per_share, 2)
        target_1r = round(spot + risk_per_share, 2)
        target_2r = round(spot + (2.0 * risk_per_share), 2)
        calculated_ext_r = round((spot - orb_high) / risk_per_share, 2) if risk_per_share > 0 else 0.0
        vwap_dist = round((spot - vwap) / risk_per_share, 2) if risk_per_share > 0 else 0.0
    else:
        stop_ref = round(spot + risk_per_share, 2)
        target_1r = round(spot - risk_per_share, 2)
        target_2r = round(spot - (2.0 * risk_per_share), 2)
        calculated_ext_r = round((orb_low - spot) / risk_per_share, 2) if risk_per_share > 0 else 0.0
        vwap_dist = round((vwap - spot) / risk_per_share, 2) if risk_per_share > 0 else 0.0

    ext_r_final = extension_r if extension_r != 0.0 else calculated_ext_r
    ...
    return {
        ...
        "hero_version": "HERO_V2.1_WYCKOFF_BROOKS",
        "vic_version": "VIC_V2.0_MACRO_GATE",
        "market": {
            "spot": round(spot, 2),
            "orb_high": round(orb_high, 2),
            "orb_low": round(orb_low, 2),
            "orb5_high": round(orb5_high, 2) if orb5_high > 0 else round(orb_high, 2),
            "orb5_low": round(orb5_low, 2) if orb5_low > 0 else round(orb_low, 2),
            "orb15_high": round(orb15_high, 2) if orb15_high > 0 else round(orb_high, 2),
            "orb15_low": round(orb15_low, 2) if orb15_low > 0 else round(orb_low, 2),
            "accepted_or_high": round(accepted_or_high, 2) if accepted_or_high > 0 else round(orb_high, 2),
            "accepted_or_low": round(accepted_or_low, 2) if accepted_or_low > 0 else round(orb_low, 2),
            "orb_width_atr": round((orb_high - orb_low) / max(atr_15m, 0.01), 2),
            "extension_r": round(ext_r_final, 2),
            "orb_extension_atr": round(ext_r_final, 2),
...
def resolve_candidate_path(
    ...
    # Calculate VIC counterfactual intervention value
    vic_verdict = candidate.get("vic", {}).get("verdict", "PERMIT")
    vic_scalar = candidate.get("vic", {}).get("sizing_scalar", 1.0)
    if vic_verdict == "VETO":
        vic_intervention_value_r = -cf_result  # Positive if it prevented a loss
    elif vic_scalar < 1.0:
        vic_intervention_value_r = round((1.0 - vic_scalar) * (-cf_result), 2)
    else:
        vic_intervention_value_r = 0.0
    ...
    out["vic_intervention_value_r"] = vic_intervention_value_r
