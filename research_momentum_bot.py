...
def run_continuous_risk_loop():
  """Checks stops/targets without spamming API endpoints."""
  ...
  if now_str >= FLATTEN_TIME_STR:
    try:
      ...
      if positions:
        print(
            f"\n🛑 [EOD CUTOFF] {now_str} >= {FLATTEN_TIME_STR}. Liquidating"
            " intraday trades."
        )
  ...
  for sym in list(active_trades.keys()):
    ...
    try:
      ...
      if hit_stop or hit_target:
        ...
        # Log completion to research logger
...
def scan_alpha_signals():
  """Evaluates 30m ORB signals and logs full shadow attribution."""
  now = datetime.now()
  ...
  try:
    ...
    if len(positions) >= MAX_CONCURRENT_POSITIONS:
      print(
          f"⚪ Max positions ({MAX_CONCURRENT_POSITIONS}) reached. Skipping"
          " entries."
      )
      return
  except Exception as e:
  ...
  for ticker in UNIVERSE:
    ...
    if is_bullish:
      match = select_research_contract(ticker, spot, ContractType.CALL)
      if match:
        ...
        print(
            f"🚀 [BULLISH ORB] {ticker} | Spot: ${spot:.2f} | Alpha Score:"
            f" {alpha_score}/100"
        )
        print(
            f"   Selected: {match['symbol']} (Delta: {match['delta']:.2f}, Mid:"
            f" ${match['mid']:.2f})"
        )
