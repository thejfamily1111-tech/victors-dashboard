...
class BearPutExecutor:
    ...
    def __init__(self, ticker='QQQ', state=None, rsi_min=30.0, rsi_max=50.0):
        if ticker != 'QQQ':
            raise ValueError('BEAR supports QQQ only')
        ...
        self.ticker = ticker
        self.state = {'version': 1, 'strategy': 'QQQ_PUT', 'submitted_signals': [], 'positions': {}}
        if state is not None:
            import copy
            if state.get('strategy') != 'QQQ_PUT' or state.get('version') != 1 or not isinstance(state.get('positions'), dict) or not isinstance(state.get('submitted_signals'), list):
                raise ValueError('Invalid BEAR state')
    ...
    def check_put_execution_triggers(self, market_data_5m, mag_7_status):
        ...
        return self.evaluate_entry(market_data_5m, mag_7_status)['action'] == 'BUY_PUT_SIGNAL'

    def mark_entry_submitted(self, signal_id):
        if not signal_id or not signal_id.startswith('QQQ-PUT-'):
    ...
    def evaluate_exit(self, position_id, data, now=None):
        ...
        try:
            ...
            reason = 'CLOSE_ABOVE_EMA21' if close > ema21 else None
            if p['runner_active'] and close > ema9: reason = 'RUNNER_CLOSE_ABOVE_EMA9'
            if reason:
                p['close_reason'] = reason
                return decision('CLOSE_ALL', reason, p['remaining_qty'])
            ...
            if not p['scale_requested']:
                ...
                if all(k in data for k in ('upper_bband','lower_bband','previous_bb_width')):
                    ...
                    stretched = close < lower and upper-lower >= 1.10*previous_width
...
if __name__ == '__main__':
    print('BEAR rule module loaded. No broker connection or automatic trading loop is configured.')
